    ...
    def minMeetingRooms(self, intervals: List[Interval]) -> int:
        ...
        while start_index < len(curr_start):
            ...
            #3)if curr_start larger_than curr_end
                ...
                #3.1)how many interval overlap with curr_start
                #3.2)delete curr_end's interval
                #3.2)curr_start's interval cant_overlap curr_end's interval / curr_start's interval align with curr_end's interval 
                room -= 1
                #3.3)curr_end move to next_end(run untill curr_start smaller than curr_end) 
                end_index += 1 
        return max_rooms