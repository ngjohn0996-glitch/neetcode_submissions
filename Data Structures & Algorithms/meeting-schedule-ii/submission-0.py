    ...
    def minMeetingRooms(self, intervals: List[Interval]) -> int:
        ...
        while start_index < len(curr_start):
            ...
            if curr_start[start_index] < curr_end[end_index]:
                ...
                room += 1
                #2.2)curr_start move to next_start, next_start compare with curr_end(find untill all overlapping_interval at curr_end)
                start_index += 1
                max_rooms = max(max_rooms, room)

            #3)if curr_start larger_than curr_end
            else:
                #3.1)curr_end move to next_end, curr_start compare with next_end(no more overlapping_interval at curr_end) 
                end_index += 1 
