    ...
    def minMeetingRooms(self, intervals: List[Interval]) -> int:
        ...
        while start_index < len(curr_start):
            ...
            #3)if curr_start larger_than curr_end
                ...
                #3.2)curr_end move to next_end(run untill curr_start smaller than curr_end) 
                end_index += 1 
                #3.3)delete curr_end's interval
                #3.3)curr_end's interval cant_overlap curr_start's interval(curr_end's interval align with curr_start's interval)
                room -= 1
        return max_rooms